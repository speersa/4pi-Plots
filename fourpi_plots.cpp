...
void plotMomCos(Experiment exp, const char * accum_levels[], const char * sample_names[], const char * categories[], const int topology_branch[], TCanvas* canvas) {
  
  DrawingTools* draw = new DrawingTools("test.root");
  ...
  for(int j = 0; j<2; j++){ // loop over distribution and efficiency&purity
	...
	if(j==0){ //Distributions
	  ...
	  // TH1D* histo1 = (TH1D*)draw->GetLastHisto();
...
<< "outFV Integral: " << outFV->Integral(0,27) << ", " << 100*outFV->Integral(0, 27)/TotalIntegral << "%" << std::endl;
	  */
	  ...
	  if(i==0){ //Momentum
	    TLegend* legend = NULL;
	    if(k>7){legend = new TLegend(0.6,0.7,0.9,0.9);}
	    else {legend = new TLegend(0.6,0.2,0.9,0.4);}
	    ...
	    TGraphAsymmErrors* graph1 = (TGraphAsymmErrors*)draw->GetLastGraph();
...
void fourpi_plots_root6() {
  
  gROOT->SetBatch(); //Stops it from spamming windows when drawing
  DrawingTools* draw = new DrawingTools("test.root"); //Initialises DrawingTools with a random .root file
  //draw->SetStackFillStyle(3254);

  // Create an experiment with name "nd280"
  Experiment exp("nd280");

  DataSample* prod_7 = new DataSample("/home/t2k/aspeers/4pi_selection/test.root"); //test.root");
  ...
  plotMomCos(exp, accum_levels, sample_names, categories, topology_branch, canvas);
